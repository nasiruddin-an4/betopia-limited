...
import { ManagedwhyBetopiaFeatures } from "@/data/service";
    ...
    <div className="bg-[#f7f5ff] py-10 lg:py-20">
      <Container>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-20 items-stretch">
          ...
          <div>
            <HeadingThree
              title="Why Betopia Limited for Managed Services"
              className="text-[#0A0A0A]"
              center={false}
            />
            <HeadingSix
              title="Operational excellence. Zero distractions."
              className="text-[#0A0A0A]"
              center={false}
            />

            <div className="mt-10 space-y-7">
              {ManagedwhyBetopiaFeatures.map((item, index) => (
                <WhyBetopiaFeature
                  key={index}
                  icon={item.icon}
                  text={item.text}
