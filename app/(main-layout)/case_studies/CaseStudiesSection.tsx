...
export default function CaseStudiesSection() {
  ...
  const filteredData = caseStudies
    //  Only Customer Story
    .filter((item) => item.category === "Case Studies")
    //  Existing filters
    .filter((item) =>
  ...
  return (
    <Container>
      <section className="py-10 lg:py-20  lg:space-y-8">
        <h2 className="text-2xl md:text-4xl lg:text-[52px] font-bold text-[#45556C] mb-3 lg:mb-4">
          Case Studies
        </h2>
        ...
        hover:scale-105 hover:shadow-xl active:scale-95 cursor-pointer"
            ...
            <div className="flex justify-center pt-6">
              <button
                onClick={() => setVisibleCount((prev) => ITEMS_PER_LOAD)}
                className="inline-flex items-center gap-2 rounded-full px-8 py-3
        text-sm font-semibold text-white
        bg-linear-to-r from-[#FF9500] to-[#F54900]
        shadow-lg transition-all duration-300
